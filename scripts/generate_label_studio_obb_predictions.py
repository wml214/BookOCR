"""使用 YOLOv8-OBB 模型为 Label Studio 未完成任务生成旋转框预标注。

本脚本专门服务于书脊 OBB 路线：模型输出每个书脊的 4 个角点，写入
Label Studio 后表现为可编辑的四边形区域。相比 seg mask 轮廓，OBB 预标注
点数固定、形状清爽，更适合人工快速修正倾斜书脊。

脚本默认只处理未人工提交的任务，不会覆盖已经 Submit 的人工标注。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="为 Label Studio 生成 OBB 书脊预标注。")
    parser.add_argument(
        "--database",
        type=Path,
    ...
    parser.add_argument(
        "--model",
        type=Path,
        default=PROJECT_ROOT / "models/weights/book_spine_obb_v1_100_best.pt",
        help="用于生成预标注的 YOLOv8-OBB 权重。",
    )
    parser.add_argument(
        "--images-root",
        type=Path,
    ...
    parser.add_argument("--conf", type=float, default=0.60, help="预测置信度阈值。")
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="仅处理前 N 个任务；0 表示处理全部，用于小批量试运行。",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def obb_to_label_studio_result(
    points_xy: np.ndarray,
    task: TaskInfo,
    score: float,
) -> dict[str, Any] | None:
    """把单个 OBB 四点框转换成 Label Studio polygonlabels result。"""
...
def predict_task(model: YOLO, task: TaskInfo, args: argparse.Namespace) -> tuple[list[dict[str, Any]], float]:
    ...
    confidences = prediction.obb.conf.detach().cpu().numpy().tolist()

    results: list[dict[str, Any]] = []
    used_scores: list[float] = []
    for points_xy, confidence in zip(points_array, confidences, strict=False):
        item = obb_to_label_studio_result(
...
def backup_database(database: Path, tag: str) -> Path:
    """写入 prediction 前备份数据库。"""

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = database.with_name(f"{database.stem}_{tag}_{timestamp}{database.suffix}")
...
def write_predictions(
    ...
    for task_id, (task_project_id, result, score) in predictions.items():
        cursor.execute(
            """
...
def main() -> None:
    ...
    if not args.model.exists():
        raise FileNotFoundError(f"找不到 OBB 模型权重：{args.model}")
    ...
    print(f"准备为 {len(tasks)} 个任务生成 OBB 预标注。")
    ...
    backup_path = backup_database(args.database, tag="before_obb_predictions")
    ...
    print("已写入 Label Studio prediction 表。请刷新 Label Studio 页面查看 OBB 预标注。")
